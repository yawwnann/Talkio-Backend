...
async function main() {
  ...
  const diagnosesData = [
    {
      ...
      symptoms: JSON.stringify(["tidak_menyapa", "tidak_menatap_mata", "kosakata_terbatas", "tidak_menunjuk"]),
      riskLevel: "HIGH",
      score: 0.85,
    ...
    },
    {
      ...
      riskLevel: "MEDIUM",
      score: 0.45,
      recommendation: "Perlu observasi lebih lanjut. Pantau perkembangan anak secara berkala."
    },
    {
    ...
    },
    {
      ...
      riskLevel: "LOW",
      score: 0.15,
      recommendation: "Terus pantau perkembangan bicara anak. Tidak ada risiko terdeteksi."
    },
    {
      childId: children[4].id,
      symptoms: JSON.stringify(["terlambat_bicara", "tidak_bersosialisasi", "kosakata_terbatas"]),
      riskLevel: "MEDIUM",
      score: 0.55,
      recommendation: "Observasi diperlukan. Pertimbangkan stimulasi bicara di rumah."
    }
  ];
  ...
  const [diagnoses] = await connection.query(`SELECT id, childId, riskLevel FROM diagnoses`);
  ...
  const [mlPredictions] = await connection.query(`SELECT COUNT(*) as count FROM ml_predictions`);
  ...
  for (const session of therapySessionsData) {
    await connection.execute(`
      ...
      VALUES (UUID(), ?, ?, ?, ?, ?, ?, NOW(), NOW())
    `, [session.childId, session.therapistId, session.schedule, session.therapyType, session.paymentStatus, session.isActive]);
  }

  const [sessions] = await connection.query(`SELECT COUNT(*) as count FROM therapy_sessions`);
  console.log(`   ✓ ${sessions[0].count} therapy sessions created\n`);

  // 6. Seed Game Logs
  console.log("🎮 Seeding game logs...");

  const gameTypes = ["Kata Bergambar", "Suara Binatang", "Latihan Artikulasi", "Cerita Interaktif", "Tebak Suara"];
  ...
  const [gameLogs] = await connection.query(`SELECT COUNT(*) as count FROM game_logs`);
  ...
  const [progressUploads] = await connection.query(`SELECT COUNT(*) as count FROM progress_uploads`);
  ...
  const educationData = [
    {
      ...
      content: "Artikel lengkap tentang milestone perkembangan bicara anak dari usia 0-5 tahun...",
      type: "ARTICLE",
      authorId: admin.id
    },
    {
      title: "Cara Stimulasi Bicara di Rumah",
      content: "Tips dan teknik stimulasi bicara yang bisa dilakukan orang tua di rumah...",
      type: "ARTICLE",
      authorId: admin.id
    },
    {
    ...
    },
    {
      ...
      content: "Kenali tanda-tanda awal speech delay pada anak dan kapan harus ke profesional...",
      type: "ARTICLE",
  ...
  const [educationContents] = await connection.query(`SELECT COUNT(*) as count FROM education_contents`);
  console.log(`   ✓ ${educationContents[0].count} education contents created\n`);
